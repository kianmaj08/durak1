...
export async function POST(req: NextRequest) {
  ...
  try {
    if (action === "create") {
      ...
      if (!name || !playerId) return NextResponse.json({ error: "name und playerId noetig" }, { status: 400 });
    ...
    if (action === "join") {
      ...
      if (state.phase !== "lobby") return NextResponse.json({ error: "Spiel laeuft bereits" }, { status: 400 });
